...
predictions <- predict(final_model,data) %>% 
  ...
  .[,`:=`(
    home_outcome = case_when(outcome=="home_win"~1,TRUE~0)
    ,away_outcome = case_when(outcome=="away_win"~1,TRUE~0)
...
gini_tab <- rbind(
predictions[,.(away_gini=auc(away_outcome,away_win),home_gini=auc(home_outcome,home_win),tie_gini=auc(tie_outcome,tie)),year] %>% .[,cat:="all"]
,predictions[train_data==1]%>%.[,.(away_gini=auc(away_outcome,away_win),home_gini=auc(home_outcome,home_win),tie_gini=auc(tie_outcome,tie)),year] %>% .[,cat:="train"]
,predictions[train_data==0]%>%.[,.(away_gini=auc(away_outcome,away_win),home_gini=auc(home_outcome,home_win),tie_gini=auc(tie_outcome,tie)),year] %>% .[,cat:="test"]
)

cal <- rbind(
  predictions[,.(away_predicted=mean(away_win),away_actual=sum(away_outcome)/.N,away_vol=sum(away_outcome)
                      ,home_predicted=mean(home_win),home_actual=sum(home_outcome)/.N,home_vol=sum(home_outcome)
                      ,tie_predicted=mean(tie),tie_actual=sum(tie_outcome)/.N,tie_vol=sum(tie_outcome)
                      )]%>% .[,cat:="all"]
  ,predictions[train_data==1]%>%.[,.(away_predicted=mean(away_win),away_actual=sum(away_outcome)/.N,away_vol=sum(away_outcome)
                      ,home_predicted=mean(home_win),home_actual=sum(home_outcome)/.N,home_vol=sum(home_outcome)
                      ,tie_predicted=mean(tie),tie_actual=sum(tie_outcome)/.N,tie_vol=sum(tie_outcome)
                      )]%>% .[,cat:="train"]
  ,predictions[train_data==0]%>%.[,.(away_predicted=mean(away_win),away_actual=sum(away_outcome)/.N,away_vol=sum(away_outcome)
                      ,home_predicted=mean(home_win),home_actual=sum(home_outcome)/.N,home_vol=sum(home_outcome)
                      ,tie_predicted=mean(tie),tie_actual=sum(tie_outcome)/.N,tie_vol=sum(tie_outcome)
                      )]%>% .[,cat:="test"]
)
