rankings_shiny_graph <- function(df,home,away){
  ...
  data_temp <- data %>%
    ...
    .[,text:= paste0("Country: ", team, "<br>", 
                   "Rank: ", rank, "<br>")]%>%
    .[, text_x := -points + ifelse(points > 1000, 80, -160)]
  ...
  plotly::ggplotly(
    data_temp %>%
      ...
      geom_text(data=data_temp[team==away],aes(x=text_x+ifelse(points>1000,80,-160),y=-0.115,label=sprintf("%s rank:\n#%s (%spts)",away,rank,points)),color="#a2eacb",size=4,hjust=0)+
      geom_text(data=data_temp[team==home],aes(x=text_x,y=-0.15,label=sprintf("%s rank:\n#%s (%spts)",home,rank,points)),color="#551fbd",size=4,hjust=0)+
